...
class NoseROIDataset(Dataset):
    ...
    def __init__(self, split: str, limit: Optional[int] = None, image_size: int = 128, augment: bool = False):
        ...
        # Prefer a native-resolution source directory when available so we
        # don't throw away pixels by upsampling from a smaller crop. Falls
        # back to nose_roi_128 for backwards compatibility.
        candidates = [
            ARTIFACTS_DIR / "dataset" / f"nose_roi_{image_size}",
            ARTIFACTS_DIR / "dataset" / "nose_roi_256",
            ARTIFACTS_DIR / "dataset" / "nose_roi_128",
        ]
        self.roi_dir = next((c for c in candidates if c.exists() and any(c.iterdir())), candidates[-1])
        self.items = load_pairs(split=split, limit=limit)
        # Filter to items that have nose ROI files
        self.items = [item for item in self.items if (self.roi_dir / f"{item.sample_id}_pre.jpg").exists()]
        self.image_size = image_size
        self.augment = augment and split == "train"
        self.base_transform = transforms.Compose(
            [
