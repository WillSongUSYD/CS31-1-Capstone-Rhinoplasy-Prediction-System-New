...
| 可用磁盘空间 | 约 6 GB（应用 + 下载的 AI 模型） | 约 6 GB（应用 + 下载的 AI 模型） |
...
1. 下载 Windows 版 `.zip` 并解压到任意位置（例如桌面）。
2. 打开解压后的文件夹，其中包含：
...
3. 请将以上**所有**项目保持在同一文件夹内。若缺少 `_internal` 文件夹，
   应用将无法启动。
4. 双击 **`CS31-1-Rhinoplasty-Prediction-Studio.exe`** 运行。首次运行时
